...
bool ZipExtractor::ExtractWithShell(const std::wstring& zipPath, const std::wstring& destinationPath)
{
    ...
    // Wait for extraction to complete - use longer sleep and retry count
    if (SUCCEEDED(hr)) {
        Sleep(5000); // Wait 5 seconds for extraction
    }
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
                ...
                wrapperPath = tempExtractPath + L"\\" + findData.cFileName;
